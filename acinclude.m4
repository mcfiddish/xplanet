...
AC_DEFUN([AC_FIND_FREETYPE2],
[
AC_ARG_WITH(freetype2,AC_HELP_STRING([--with-freetype2],[Enable Freetype2 support for TrueType fonts (YES)]))

have_freetype2='no'
if test "$with_freetype2" != 'no'; then
   AC_PATH_PROG(PKG_CONFIG, pkg-config, no)
   if test "$PKG_CONFIG" = no; then
      AC_MSG_WARN(*** Xplanet will be built without freetype2 support ***)
   else
      PKG_CHECK_MODULES([FREETYPE2], freetype2 >= 2.0.0, have_freetype2='yes', have_freetype2='no')
      AC_SUBST(FREETYPE_CFLAGS)
      AC_SUBST(FREETYPE_LIBS)
      if test "$have_freetype2" = 'yes'; then
        AC_DEFINE(HAVE_LIBFREETYPE,,Define if you have freetype2)
      fi
   fi
fi
])
